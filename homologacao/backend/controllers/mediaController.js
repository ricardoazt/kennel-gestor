...
const { generateThumbnail, generateVideoThumbnail, getImageMetadata, optimizeImage, optimizeAndResizeImage, optimizeVideo } = require('../utils/imageProcessor');
...
const uploadMedia = async (req, res) => {
    try {
        ...
        if (fileType === 'image') {
            try {
                // Optimize and resize image (replaces original)
                const optimizedPath = file.path + '.optimized.jpg';
                const optimizationResult = await optimizeAndResizeImage(file.path, optimizedPath);
                ...
                console.log(`Image optimized: ${optimizationResult.originalSize} → ${optimizationResult.size} bytes (${optimizationResult.compressionRatio}% reduction)`);
            ...
            try {
                // Optimize video (replaces original)
                const optimizedPath = file.path + '.optimized.mp4';
                const optimizationResult = await optimizeVideo(file.path, optimizedPath);
                ...
                console.log(`Video optimized: ${optimizationResult.originalSize} → ${optimizationResult.size} bytes (${optimizationResult.compressionRatio}% reduction)`);
                ...
                // If video optimization fails, still try to generate thumbnail
                try {
                    const thumbnailFilename = `thumb-${file.filename}.jpg`;
                    const thumbnailFullPath = path.join(path.dirname(file.path), thumbnailFilename);
                    await generateVideoThumbnail(file.path, thumbnailFullPath);
        ...
        // Create database record with optimized file size
        const mediaFile = await MediaFile.create({
            ...
            file_size: optimizedSize, // Use optimized size
        ...
        for (const file of req.files) {
            ...
            if (fileType === 'image') {
                try {
                    ...
                    console.log(`Image optimized: ${file.originalname} - ${optimizationResult.compressionRatio}% reduction`);
                ...
                try {
                    ...
                    console.log(`Video optimized: ${file.originalname} - ${optimizationResult.compressionRatio}% reduction`);
